...
const findMerchant: NextApiHandler<MerchantInfo | undefined> = async (request, response) => {
    // If get request
    if (request.method === 'GET') {
        ...
        if (merchant) {
            ...
            response.status(200).json({ index: id, address, company, currency, maxValue, location });
        } else {
